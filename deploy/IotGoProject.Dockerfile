...
RUN cd iot-go-project && go mod tidy && swag init --parseDependency --parseInternal --parseDepth 5 --instanceName "swagger" && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o main .
