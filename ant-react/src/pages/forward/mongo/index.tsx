import MongoTransmitUpdateForm from '@/pages/forward/mongo/MongoTransmitUpdateForm';
import {
  addMongoTransmit,
  deleteMongoTransmit,
  MongoTransmitPage,
  updateMongoTransmitPage,
...
const handleAdd = async (fields: API.MongoTransmitListItem) => {
  const hide = message.loading('正在添加');
  try {
    await addMongoTransmit({ ...fields });
    hide();
...
const handlerUpdate = async (fields: API.MongoTransmitListItem) => {
  const hide = message.loading('正在更新');
  try {
    await updateMongoTransmitPage({ ...fields });
    hide();
...
const Admin: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.MongoTransmitListItem>();

  const columns: ProColumns<API.MongoTransmitListItem>[] = [
    {
      key: 'ID',
  ...
  return (
    <PageContainer>
      <ProTable<API.MongoTransmitListItem, API.PageParams>
      ...
      <ModalForm
        ...
        onFinish={async (value) => {
          const success = await handleAdd(value as API.MongoTransmitListItem);
          if (success) {
      ...
      >
        {currentRow?.ID && (
          <ProDescriptions<API.MongoTransmitListItem>
            column={2}
            title={currentRow?.name}
            request={async () => ({
              data: currentRow || {},
            })}
            columns={columns as ProDescriptionsItemProps<API.MongoTransmitListItem>[]}
          />
        )}
      </Drawer>
    </PageContainer>
