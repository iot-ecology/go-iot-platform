...
const handleAdd = async (fields: API.CoapHandlerListItem) => {
  const hide = message.loading('正在添加');
  try {
    await addCoapHandler({ ...fields });
    hide();
...
const handlerUpdate = async (fields: API.CoapHandlerListItem) => {
  const hide = message.loading('正在更新');
  try {
    await updateCoapHandler({ ...fields });
    hide();
...
const Admin: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.CoapHandlerListItem>();
  ...
  const columns: ProColumns<API.CoapHandlerListItem>[] = [
    {
      key: 'ID',
  ...
  return (
    <PageContainer>
      <ProTable<API.CoapHandlerListItem, API.PageParams>
      ...
      >
        {currentRow?.ID && (
          <ProDescriptions<API.CoapHandlerListItem>
            column={2}
            title={currentRow?.name}
            request={async () => ({
              data: currentRow || {},
            })}
            columns={columns as ProDescriptionsItemProps<API.CoapHandlerListItem>[]}
          />
        )}
      </Drawer>
    </PageContainer>
