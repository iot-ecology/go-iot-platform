import WsHandlerUpdateForm from '@/pages/Protocol/ws/clients/WsHandlerUpdateForm';
import {
  addWebsocketHandler,
  deltedWebsocketHandler,
  deviceList,
  mqttScriptCheck,
  updateWsHandler,
  WebsocketHandlerPage,
...
const handleAdd = async (fields: API.WsHandlerListItem) => {
  const hide = message.loading('正在添加');
  try {
    await addWebsocketHandler({ ...fields });
    hide();
...
const handleRemove = async (id: any) => {
  ...
  try {
    await deltedWebsocketHandler(id);
    hide();
...
const handlerUpdate = async (fields: API.WsHandlerListItem) => {
  const hide = message.loading('正在更新');
  try {
    await updateWsHandler({ ...fields });
    hide();
...
const Admin: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.WsHandlerListItem>();
  ...
  const columns: ProColumns<API.WsHandlerListItem>[] = [
    {
      key: 'ID',
  ...
  return (
    <PageContainer>
      <ProTable<API.WsHandlerListItem, API.PageParams>
      ...
      <ModalForm
        ...
        onFinish={async (value) => {
          const success = await handleAdd(value as API.WsHandlerListItem);
          if (success) {
      ...
      >
        {currentRow?.ID && (
          <ProDescriptions<API.WsHandlerListItem>
            column={2}
            title={currentRow?.ID}
            request={async () => ({
              data: currentRow || {},
            })}
            columns={columns as ProDescriptionsItemProps<API.WsHandlerListItem>[]}
          />
        )}
      </Drawer>
    </PageContainer>
