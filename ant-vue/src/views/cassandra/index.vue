...
import { CASSANDRA_CREATE, CASSANDRA_DELETE, CASSANDRA_PAGE, CASSANDRA_UPDATE } from "@/api";
...
const { pageList } = useQueryPage({ path: CASSANDRA_PAGE, type: "get" }, pagination, list);

const { onAddData, onCancelData } = useAddPage({ path: CASSANDRA_CREATE, type: "post" }, formRef, form, modalVisible, pageList);

const { save } = useUpdatePage({ path: CASSANDRA_UPDATE, type: "post" }, list, editableData, pageList);

// const { deleteById } = useDeletePage({ path: CASSANDRA_DELETE, type: "delete" }, pageList);
