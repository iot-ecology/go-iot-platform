    ...
    <a-card :bordered="true">
      ...
      <a-table :columns="columns" :data-source="list" bordered :pagination="pagination" @change="handleTableChange">
        <template #bodyCell="{ column, text, record }">
          <template v-if="['name', 'host', 'token'].includes(String(column.dataIndex))">
            <div>
              ...
              <template v-else>
                <a-input-password v-if="column.dataIndex === 'token' || column.dataIndex === 'host'" :value="text" :bordered="false" />
      ...
      >
        <a-form ref="formRef" :label-col="{ style: { width: '140px' } }" :labelWrap="true" :rules="rules" :model="form">
          ...
          <a-form-item :label="$t('message.token')" name="token">
            <a-input-password v-model:value="form.token" style="width: 250px" />
          </a-form-item>
        </a-form>
      </a-modal>
    </a-card>
...
import { INFLUXDB_CREATE, INFLUXDB_DELETE, INFLUXDB_PAGE, INFLUXDB_UPDATE } from "@/api";
...
const { rules } = useRoles({
  ...
  token: [{ required: true, message: "message.token", trigger: "blur" }],
});
...
const { pageList } = useQueryPage({ path: INFLUXDB_PAGE, type: "get" }, pagination, list);

const { onAddData, onCancelData } = useAddPage({ path: INFLUXDB_CREATE, type: "post" }, formRef, form, modalVisible, pageList);

const { save } = useUpdatePage({ path: INFLUXDB_UPDATE, type: "post" }, list, editableData, pageList);

// const { deleteById } = useDeletePage({ path: INFLUXDB_DELETE, type: "delete" }, pageList);
