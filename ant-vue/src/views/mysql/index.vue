...
import { MYSQL_CREATE, MYSQL_DELETE, MYSQL_PAGE, MYSQL_UPDATE } from "@/api";
...
const { pageList } = useQueryPage({ path: MYSQL_PAGE, type: "get" }, pagination, list);

const { onAddData, onCancelData } = useAddPage({ path: MYSQL_CREATE, type: "post" }, formRef, form, modalVisible, pageList);

const { save } = useUpdatePage({ path: MYSQL_UPDATE, type: "post" }, list, editableData, pageList);

// const { deleteById } = useDeletePage({ path: MYSQL_DELETE, type: "delete" }, pageList);
