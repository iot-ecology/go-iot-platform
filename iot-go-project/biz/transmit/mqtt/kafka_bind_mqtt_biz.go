...
func (biz *KafkaTransmitBindBiz) Bind(req models.KafkaTransmitBind) {
	if req.Enable {
		...
		// 缓存构造
		glob.GRedis.LPush(context.Background(), "transmit:Kafka:"+req.Protocol +":"+string(req.DeviceUid) +":" +req.IdentificationCode, jsonData)
	}
}
...
func (biz *KafkaTransmitBindBiz) HandlerRedis(req models.KafkaTransmitBind) {
	if req.Enable {
		...
		glob.GRedis.LRem(context.Background(), "transmit:Kafka:"+req.Protocol +":"+string(req.DeviceUid) +":" +req.IdentificationCode, 1, biz.toByte(req))
	}
}
